...
def main_train_surrogate_for_victim_clf(experiment: str, ds_local_path: str, device: str,
                                        batch_size: int, lr: float, n_epochs: int,
                                        net_victim_local_dir: str, ds_victim_local_path: str = None):
    print('-' * NUM_OF_HYPHENS)
    print('Running train surrogate for victim clf experiment...')
    ...
    ds_train_path_clf = None
    ds_val_path_clf = None
    ds_test_path_clf = None
    ds_train_clf, loader_train_clf = None, None
    ds_val_clf, loader_val_clf = None, None
    ds_test_clf, loader_test_clf = None, None
    if ds_victim_local_path:
        print('load victim datasets to pytorch')
        ds_train_path_clf = os.path.join(ds_victim_local_path, 'train')
        ds_val_path_clf = os.path.join(ds_victim_local_path, 'val')
        ds_test_path_clf = os.path.join(ds_victim_local_path, 'test')
        ds_train_clf, loader_train_clf = create_ds_loader(path=ds_train_path_clf, transform=train_transform, batch_size=batch_size)
        ds_val_clf, loader_val_clf = create_ds_loader(path=ds_val_path_clf, transform=train_transform, batch_size=batch_size)
        ds_test_clf, loader_test_clf = create_ds_loader(path=ds_test_path_clf, transform=test_transform, batch_size=batch_size)
    ...
    for epoch in range(1, n_epochs+1):
        ...
        results_df, results_deep_df = process_epoch_surrogate(experiment=experiment,
                                                              ...
                                                              scheduler=scheduler,
                                                              loaders_victim=[loader_train_clf, loader_val_clf,
                                                                              loader_test_clf]
                                                              )
    ...
    print('Finished running train surrogate for victim clf experiment!')
