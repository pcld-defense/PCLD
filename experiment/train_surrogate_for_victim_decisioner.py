...
def main_train_surrogate_for_victim_decisioner(experiment: str, ds_local_path: str, device: str,
                                               batch_size: int, lr: float, n_epochs: int,
                                               ds_decisioner_local_path: str):
    print('-' * NUM_OF_HYPHENS)
    print('Running train surrogate for victim decisioner experiment...')
    ...
    print('initialize surrogate model')
    ...
    print('load decisioner model predictions')
    decisioner_labels = pd.read_csv(ds_decisioner_local_path)
    decisioner_labels = decisioner_labels.drop_duplicates()

    ds_train_decisioner = decisioner_labels[(decisioner_labels.dir == 'eps_0') &
                                            (decisioner_labels.ds_name == 'train')]
    ds_val_decisioner = decisioner_labels[(decisioner_labels.dir == 'eps_0') &
                                          (decisioner_labels.ds_name == 'validation')]
    ds_test_decisioner = decisioner_labels[(decisioner_labels.dir == 'eps_0') &
                                           (decisioner_labels.ds_name == 'test')]
    ds_train_full_decisioner = pd.concat([ds_train_decisioner, ds_val_decisioner], axis=0, ignore_index=True)
    ds_full_dataset_decisioner = pd.concat([ds_train_full_decisioner, ds_test_decisioner], axis=0, ignore_index=True)
    ...
    for epoch in range(1, n_epochs+1):
        ...
        # Train
        results_df, results_deep_df = process_epoch_surrogate_for_decisioner(
                                                              experiment=experiment,
                                                              device=device,
                                                              epoch=epoch,
                                                              net_surrogate=net_surrogate,
                                                              decisioner_labels=ds_full_dataset_decisioner,
    ...
    print('Finished running train surrogate for victim decisioner experiment!')
