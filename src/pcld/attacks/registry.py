...
Every attack is a callable with the signature::
    ...
    fn(model, x, y, *, epsilon, targeted, norm, nb_iter,
       loss_fn, nb_restarts, use_apgd) -> torch.Tensor
...
def _pgd(model, x, y, *, epsilon, targeted, norm='linf', nb_iter=10,
         loss_fn=None, nb_restarts=1, use_apgd=False, **_) -> torch.Tensor:
    """Custom PGD loop (APGD schedule, restarts, multi-step loss)."""
...
@ATTACKS.register('aa')
def _autoattack(model, x, y, *, epsilon, norm='linf', **_) -> torch.Tensor:
    """AutoAttack standard ensemble (unchanged from the legacy path)."""
