...
from LearningToPaint.baseline.DRL.actor import *
from LearningToPaint.baseline.Renderer.stroke_gen import *
...
class Arguments(typing.NamedTuple):
    ...
    divide: int
    output_dir: str
    output_canvas_dir: str
    output_img_name: str
    save_every: str
    save_canvas_every: str
    save_strokes: bool
    strokes_dir: str
    verbose: bool
...
def main(args: Arguments):
    ...
    save_every_new = []
    save_canvas_every_new = []
    if args.save_every:
        save_every_new = [int(float(e.strip())) for e in args.save_every.split(',')]
        os.makedirs(args.output_dir, exist_ok=True)
    if args.save_canvas_every:
        save_canvas_every_new = [int(float(e.strip())) for e in args.save_canvas_every.split(',')]
        os.makedirs(args.output_canvas_dir, exist_ok=True)
    canvases = []
    with torch.no_grad():
        for i in range(max_step_new):
            ...
            for j in range(5):
                ...
                if imgid_new in save_canvas_every_new:
                    canvases.append(res[j].detach().cpu().numpy())
    ...
    if save_canvas_every_new:
        print(len(canvases))
        print(len(save_canvas_every_new))
        output_path = os.path.join(args.output_canvas_dir, f'canvases_{args.output_img_name}.csv')
        # np.save(output_path, np.array(canvases))
        np.savetxt(output_path, np.array(canvases), delimiter=",", fmt='%s')
...
def _arg_parser() -> Arguments:
    ...
    parser.add_argument('--output_canvas_dir', type=str, help='path to output canvases directory')
    ...
    parser.add_argument('--save_canvas_every', type=str, help='save output canvas in every x steps')
    ...
    return Arguments(
        ...
        output_canvas_dir=parsed.output_canvas_dir,
        output_img_name=parsed.output_img_name,
        save_every=parsed.save_every,
        save_canvas_every=parsed.save_canvas_every,
